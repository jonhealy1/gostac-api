...
func bbox2polygon(bbox []float64) string {
	...
	geoString += fmt.Sprintf("[%f,%f],", bbox[0], bbox[1])
	geoString += fmt.Sprintf("[%f,%f],", bbox[2], bbox[1])
	geoString += fmt.Sprintf("[%f,%f],", bbox[2], bbox[3])
	geoString += fmt.Sprintf("[%f,%f],", bbox[0], bbox[3])
	geoString += fmt.Sprintf("[%f,%f]]]}", bbox[0], bbox[1])
	return geoString
}
...
func lineString(geom [][2]float64) string {
	...
	geoString += fmt.Sprintf("[[%f,%f],", geom[0][0], geom[0][1])
	geoString += fmt.Sprintf("[%f,%f]]}", geom[1][0], geom[1][1])
	return geoString
}
...
func polygonString(geom [][][2]float64) string {
	...
	for i := 0; i < len(geom[0])-1; i++ {
		geoString += fmt.Sprintf("[%f,%f],", geom[0][i][0], geom[0][i][1])
	}
	geoString += fmt.Sprintf("[%f,%f]]]}", geom[0][len(geom[0])-1][0], geom[0][len(geom[0])-1][1])
	return geoString
}
...
func fix3dBbox(search models.Search) []float64 {
	...
	if len(search.Bbox) == 6 {
		...
		log.Println("bbox must be of length 4 or 6")
	}
	return bbox
}
